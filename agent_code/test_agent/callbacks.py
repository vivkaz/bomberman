...
def act(self, game_state: dict) -> str:
    ...
    epsilon = max(1 - game_state["round"] / 55, 0.1)
...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
        ...
        for j, value in enumerate(neighbours_values):
            if value == 0 and len(game_state['bombs']) > 0: 
                if any([ (neighbours[j] == np.array(bomb[0])).all() for bomb in game_state['bombs']]):
                    ...
                    continue
            if value == 0 and len(game_state['others']) > 0 :
                if any([(neighbours[j] == np.array(other[3])).all() and (other[3] != game_state['self'][3]) for other in game_state['others'] ]):
                    ...
                    continue
            if value == 0 and game_state['explosion_map'][neighbours[j][0]][neighbours[j][1]] == 1:
    ...
    def attackable_opponent(pos, neighbours=[], my_pos=(0,0), me=True):
        if len(game_state['others']) > 0:
            ...
            if np.min(dist(pos, opponents)) <= MAX_DIST_NEIGHBOUR:
                ...
                neighbours_values = get_neighbours_values(neighbours)
                j = -1 # neighbour with min distance to opponent
                d = 3
                for i, neighbour in enumerate(neighbours):
                    if neighbours_values[i] == 0:
                        d_temp = dist(neighbour, opponent)
                        if d_temp < d:
                            d, j = d_temp, i
    ...
    def direction_to_escape_bomb(my_pos, bombs, neighbours, values, j):
        ...
        bomb_area = get_vh_region(bombs[index][0])
        bomb_area.append(bombs[index][0])
        bomb_area = np.array(bomb_area)
        escape_index = np.where([ (neighbour.tolist() not in bomb_area.tolist() and game_state['field'][neighbour[0]][neighbour[1]]==0) for neighbour in neighbours])[0]
        #print("bomb area", bomb_area)
        #print("neighbours", neighbours)  
        #print("escape index", escape_index, j)    
        if len(escape_index) > 0: 
            return escape_index[0] == j

        # else predict best direction
        if game_state['field'][neighbours[j][0]][neighbours[j][1]] == 0:
            region = get_neighbours(neighbours[j])
            filtered_region = list(filter(lambda field: game_state['field'][field[0]][field[1]]==0, region))
            #print(region)
            #print(filtered_region)
            #print("len field not in area",len(np.where([field not in bomb_area for field in filtered_region])[0]))
            if len(np.where([field not in bomb_area for field in filtered_region])[0]) > 0:
                return True
        ...
        #print("max d indices, j", max_d_indices, j)
    ...
    def coin_behind_crate(my_pos, crate_pos):
        if len(game_state['coins']) > 0:
            ...
            if crate_pos[1] == my_pos[1] and crate_pos[0] < my_pos[0]: # left
                for coin in coins:
                    if coin[1] == crate_pos[1] and coin[0] + 1 == crate_pos[0]:
                        return True
                ...
                for coin in coins:
                    if coin[0] == crate_pos[0] and coin[1] -1  == crate_pos[1]:
                        return True
    ...
    elif count_opponents(my_position)>0 or (safe_death(my_position) and game_state['self'][2]==True): # opponent near the agent or safe death
    ...
    #elif ( 1 in neighbours_values or count_opponents(my_position) > 0 ) and game_state['self'][2]==True and mode in [0,1]:
    elif ( 1 in neighbours_values or attackable_opponent(my_position) ) and ( mode == 0 or mode == 1 ): # and game_state['self'][2]==True:
    ...
    for j, value in enumerate(neighbours_values):
        if value == 2: # bomb or explosion
            channels.append(-1) # like a wall, danger =^ do not go there
            ...
            elif danger(neighbours[j]):
                channels.append(2) # danger

            elif attackable_opponent(neighbours[j], neighbours, my_position, False) == j:   
                channels.append(1) # depending on game mode go in this direction or drop bomb 
            
            elif collectable_coin(neighbours[j], neighbours, my_position, False)[1] == j and mode==3: # coin in region
                channels.append(1) # depending on game mode go in this direction or drop bomb
            
            elif mode==0 and ( j in destroyable_crate(neighbours) ) and not crate_dir_flag:
                crate_dir_flag = True
                channels.append(1) # depending on game mode go in this direction or drop bomb
    ...
    """ avoid oscilation """
    if channels[0] != 2 and 1 not in channels[-4:] and 0 in channels[-4:]:
        #mask = np.where([value == 0 for value in neighbours_values]) # indices of free tiles
        mask = np.where([value == 0 for value in channels[-4:]]) # indices of free tiles
        temp = [neighbours[i] for i in mask[0]]
        ...
        if len(crate_coord[0]) > 0:
            crates = np.array([crate_coord[0], crate_coord[1]]).transpose()
            distances = dist(my_position, crates)
            d_temp = np.min(distances)
            coord = crates[np.argmin(distances)]
            if d_temp < d:
                d, case = d_temp, 0

        if len(game_state['others']) > 0:
            opponents = [ other[3] for other in game_state['others'] ]
            distances = dist(my_position, opponents)
            d_temp = np.min(distances)
            coord = opponents[np.argmin(distances)]
            if d_temp < d:
                d, case = d_temp, 1

        if len(game_state['coins']) > 0:
            coins = game_state['coins']
            distances = dist(my_position, coins)
            d_temp = np.min(distances)
            coord = coins[np.argmin(distances)]
            if d_temp < d:
                d, case = d_temp, 3

        if case != -1:
            i = -1
            channels[0] = case # change mode
            if case == 0:
                distance = np.array([ dist(neighbour, coord) for neighbour in temp ])
            ...
            i = mask[0][np.argmin(distance)]
            channels[i+2] = 1 # give direction to the next object

        #print("oscilation new features",channels, "\n")
